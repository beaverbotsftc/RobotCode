...
public class AutonomousRedFarStaticV3 extends CommandRuntimeOpMode {
    ...
    public void onStart() {
        Pair<Path, Path> autoPart1 = new PathBuilder(pinpoint.getPositionAsList())
                .linearTo(new DrivetrainState(23.27, 53.40, -0.33).toList(), 0.3, 1)
                .build();

        schedule(
                new Sequential(
                        new Instant(() -> shooter.spin(3000)),
                        new Instant(() -> shooter.setHood(0.53)),
